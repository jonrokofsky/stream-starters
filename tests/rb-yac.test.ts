...
test("2026 rush weights YAC at 20% and reweights the base formula when YAC is missing", () => {
 ...
 assert.equal(score([low,{...high,"YAC/Att":10}])[0].rush,0);
 assert.equal(score([{...low,"RuYds/G":10},{...low,"YAC/Att":10}])[0].rush,35);
});
